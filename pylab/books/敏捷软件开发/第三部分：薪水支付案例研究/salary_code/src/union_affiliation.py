...
class UnionAffiliation(Affiliation):
    ...
    def calculate_deductions(self, paycheck: Paycheck):
        fridays = self.number_of_friday_in_pay_period(paycheck.pay_period_start_date, paycheck.pay_date)
        total_dues = self.dues * fridays
        for service_charge in self.service_charges:
            if paycheck.pay_period_start_date <= service_charge.get_date() <= paycheck.pay_date:
